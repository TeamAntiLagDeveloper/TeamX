...
public class EmailService : IEmailService
{
    ...
    private string BuildLicenseEmailHtml(LicenseEmailRequest request)
    {
        ...
        var expiration = WebUtility.HtmlEncode(
            request.ExpirationDate.ToString("dd/MM/yyyy", CultureInfo.GetCultureInfo("pt-BR")));

        var year = DateTime.UtcNow.Year;
        var downloadUrl = WebUtility.HtmlEncode(_options.DownloadUrl);
        ...
        return $$"""
            ...
            <body style="font-family: Arial, Helvetica, sans-serif; background:#f4f4f4; padding:30px; margin:0;">
              <div style="max-width:600px; margin:auto; background:white; padding:35px; border-radius:12px;">
                <h1 style="color:#111; text-align:center; margin-top:0;">Bem-vindo ao TeamX</h1>
                ...
                <div style="background:#111; padding:20px; border-radius:10px; text-align:center; margin:25px 0;">
                  <p style="color:#aaa; margin:0 0 8px 0; font-size:14px;">Sua chave de ativação:</p>
                  <h2 style="color:white; letter-spacing:2px; margin:0; word-break:break-all;">{{licenseKey}}</h2>
                </div>
                <p style="font-size:15px; color:#333; margin:8px 0;"><b>Produto:</b> {{productName}}</p>
                <p style="font-size:15px; color:#333; margin:8px 0;"><b>Validade:</b> {{expiration}}</p>
                ...
                <h3 style="color:#111; margin-bottom:10px;">Como ativar sua licença:</h3>
