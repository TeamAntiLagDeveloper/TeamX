...
public class WebhookController : ControllerBase
{
    ...
    public async Task<IActionResult> Eremby(CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(request.Order.Transaction_Id))
            return BadRequest(new { success = false, message = "Transaction_Id ausente" });
        ...
        {
            _logger.LogInformation(
                "Pedido já processado: {TransactionId}",
                request.Order.Transaction_Id);

            return Ok(new { success = true, message = "Venda já processada" });
        }

        // Resolve plano pelo Code do variant Eremby (seed: 112169, 112170, 112171)
        var variantId = request.Items[0].Variant_Id.ToString();

        var plan = await _context.Plans
            .AsNoTracking()
            .FirstOrDefaultAsync(
                x => x.Code == variantId && x.IsActive,
                cancellationToken);

        if (plan is null)
            return BadRequest(new { success = false, message = "Plano Eremby não configurado" });

        var product = await _context.Products
            .AsNoTracking()
            .FirstOrDefaultAsync(
                x => x.Id == plan.ProductId && x.IsActive,
                cancellationToken);
        ...
        try
        {
            var customer = await _customerService.GetOrCreateAsync(
                request.Order.Customer.Email,
                cancellationToken);
            ...
            await _orderService.UpdateLicenseAsync(order.Id, license.Id, cancellationToken);
